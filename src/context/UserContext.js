...
import { 
  ...
  deleteUser // <-- NEW: Added deleteUser
} from 'firebase/auth';
import { 
  doc, onSnapshot, updateDoc, arrayUnion, setDoc, getDoc, 
  collection, query, orderBy, limit, getDocs, deleteDoc // <-- NEW: Added deleteDoc
...
export const UserProvider = ({ children }) => {
  ...
  const deleteAccount = async () => {
    try {
      ...
      // 1. Kill the database listener FIRST to prevent permission errors
      if (unsubscribeSnapshot.current) {
        unsubscribeSnapshot.current();
        unsubscribeSnapshot.current = null;
      }
      ...
      // Firebase security requires a recent login to delete an account.
      // Catch this specifically and prompt the user to re-log.
      if (error.code === 'auth/requires-recent-login') {
        return { 
          success: false, 
          error: "For your security, please log out and log back in before deleting your account." 
        };
      }
      return { success: false, error: error.message };
    }
  ...
  const findSmartGaps = (busyEvents, startDate, daysToScan = 365) => {
    ...
    const createGap = (startMs, endMs, dateKey, label) => {
      ...
      else if (diffMins >= 45) { gapType = 'Gold'; suggestion = 'BEST TIME: Full Workout'; color = '#c59d00'; } 
      else if (diffMins >= 20) { gapType = 'Silver'; suggestion = 'Great for HIIT / Micro'; color = '#929292'; }
    ...
    try {
      ...
      if (Platform.OS === 'ios') {
        ...
        if (user) {
          ...
          // --- NEW: AUTO-BACKFILL PAST 7 DAYS OF STEPS ---
          try {
             const pastEntries = [];
             const userDoc = await getDoc(doc(db, 'users', user.uid));
             if (userDoc.exists()) {
                ...
                for (let i = 1; i <= 7; i++) {
                   const dStart = new Date(); dStart.setDate(dStart.getDate() - i); dStart.setHours(0,0,0,0);
                   const dateStr = getLocalDateString(dStart);
                   
                   // Only backfill if the date isn't already archived in the database
          ...
          } catch (backfillErr) { console.log("Backfill error:", backfillErr); }
        }
      }
 ...
 useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        // 1. Assign the listener to the ref
        unsubscribeSnapshot.current = onSnapshot(
          ...
          // 2. Add the silent error catcher here!
          (error) => {
            console.log("Listener detached quietly during logout/deletion.");
          }
        );

        // Cleanup function for when auth state changes
        return () => { 
          if (unsubscribeSnapshot.current) {
            unsubscribeSnapshot.current(); 
          }
  ...
  return (
    <UserContext.Provider value={{ 
      ...
      login, register, logout, loginWithGoogle, completeSetup, deleteAccount, // <-- NEW: Added deleteAccount
