...
const SettingsScreen = () => {
  ...
  const { 
    ...
    converters, deleteAccount // <-- Destructured deleteAccount
  } = useUser();
        ...
        { 
          ...
          onPress: async () => {
            if (deleteAccount) {
              ...
              if (res && res.success) {
                // Show the seamless logout message you requested
  ...
  const renderModalContent = () => {
    ...
    // 2. GOALS LIST (Android Only - Pre-made Options)
    ...
    // 3. PASSWORD INPUT (Both Platforms)
    if (editType === 'password') {
        return (
            <View>
                <TextInput 
  ...
  return (
    <SafeAreaView style={styles.container}>
      <TouchableWithoutFeedback onPress={() => { Keyboard.dismiss(); setActiveBottomSheet(null); }}>
        <View style={{flex: 1}}>
          ...
          <View style={styles.header}>
            <TouchableOpacity 
              onPress={() => router.back()} // <-- Safe Expo Router GoBack
      ...
      {/* GLOBAL MODAL (For Android Goals/Units, Names, Passwords) */}
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{width: '100%'}}>
              <View style={styles.modalContent} onStartShouldSetResponder={() => true}>
                  <View style={styles.modalHeader}>
                      <Text style={styles.modalTitle}>{['weight','height','volume','energy'].includes(editType) ? `Select ${editType}` : `Edit ${editType}`}</Text>
      ...
      {/* IOS BOTTOM SHEET (For DOB, Steps, Hydration, Units) */}
